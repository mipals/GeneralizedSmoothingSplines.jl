...
# Fitting splines
con_spl = SmoothingSpline(lambda = 1e-5,shape_restrictions=(:convex,))
con_mach = machine(con_spl,X,y) 
tune!(con_mach)
pos_spl = SmoothingSpline(lambda = 1e-5,shape_restrictions=(:lowerbound,))
pos_mach = machine(pos_spl,X,y) 
tune!(pos_mach)
conpos_spl = SmoothingSpline(lambda = 1e-5,shape_restrictions=(:convex,:lowerbound))
